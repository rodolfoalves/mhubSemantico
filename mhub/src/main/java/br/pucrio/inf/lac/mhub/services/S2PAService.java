...
public class S2PAService extends Service implements TechnologyListener {
    ...
    private void bootstrap() {
        ...
        Technology tempBLE = new BLETechnology( ac );
        exist = tempBLE.initialize();
        if( exist ) {
            technologies.append(TechnologyID.BLE.id, tempBLE );
            tempBLE.setListener( this );
            tempBLE.enable();
        }

        Technology tempBT = BTTechnology.getInstance(ac);
        exist = tempBT.initialize();
        if( exist ) {
            technologies.append(TechnologyID.BT.id, tempBT);
            tempBT.setListener( this );
            tempBT.enable();
            tempBT.startScan(true);
        }

        Technology tempInt = new InternalTechnology( ac );
        exist = tempInt.initialize();
        if( exist ) {
            technologies.append( TechnologyID.INTERNAL.id, tempInt );
            tempInt.setListener( this );
            tempInt.enable();
        }
    ...
    public void onMObjectValueRead( MOUUID mobileObject, Double rssi, String serviceName, Double[] values ) {
        this.onMObjectValueRead(mobileObject, rssi, serviceName, values, null);
    }

    @Override
    public void onMObjectValueRead( MOUUID mobileObject, Double rssi, String serviceName, Double[] values, Map<String, String> properties) {
